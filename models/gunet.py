# -*- coding: utf-8 -*-
"""
Created on Wed Oct 18 12:25:17 2023
...
class gUNet(nn.Module):
	def __init__(self, kernel_size=5, base_dim=32, depths=[4, 4, 4, 4, 4, 4, 4], conv_layer=ConvLayer, norm_layer=nn.BatchNorm2d, gate_act=nn.Sigmoid, fusion_layer=SKFusion):
		...
		# input convolution
		self.inconv = PatchEmbed(patch_size=1, in_chans=172, embed_dim=embed_dims[0], kernel_size=3)
		...
		# output convolution
		self.outconv = PatchUnEmbed(patch_size=1, out_chans=172, embed_dim=embed_dims[-1], kernel_size=3)
	...
	def encode(self, x):
		...
		for i in range(self.half_num):
			...
			skips_output.append(self.skips[i](feat))
			feat = self.downs[i](feat)

		return feat, skips_output
	
	def fuse_q(self, feat, q_feat=None, feat_attn=None, q_feat_attn=None):
		feat = self.layers[self.half_num](feat)		
		return feat * feat_attn + q_feat * q_feat_attn if q_feat is not None else feat
	
	def decode(self, x, feat, skips_output):
		for i in range(self.half_num-1, -1, -1):
			feat = self.ups[i](feat)
			feat = self.fusions[i]([feat, skips_output[i]])
...
class QGUnet(nn.Module):
	BOTTLENECK_DIM = 192  # Bottleneck dimension for the gUNet
	BOTTLENECK_HW = 32
	def __init__(self, base_dim=24, gu_pretrained=False):
		super(QGUnet, self).__init__()
		self.gunet = gUNet(kernel_size=5, base_dim=base_dim, depths=[16, 16, 16, 32, 16, 16, 16], conv_layer=ConvLayer, norm_layer=nn.BatchNorm2d, gate_act=nn.Sigmoid, fusion_layer=SKFusion)
		if gu_pretrained:
			self.gunet.load_state_dict(torch.load('/home/q36131207/DehazeFormer/saved_models/indoor/QHSID-gunet.pth'), strict=False)
		for params in self.gunet.parameters():
			params.requires_grad = False
		self.gunet.eval()  # Set gUNet to eval mode
		self.qmodule = Qmodule(out_height=QGUnet.BOTTLENECK_HW, out_width=QGUnet.BOTTLENECK_HW, mode='bilinear')
		self.q_feat_attn = nn.Parameter(torch.rand(1, QGUnet.BOTTLENECK_DIM, 1, 1), requires_grad=True)
		self.feat_attn = nn.Parameter(torch.rand(1, QGUnet.BOTTLENECK_DIM, 1, 1), requires_grad=True)

	def forward(self, x):
		feat, output_skips = self.gunet.encode(x)
		q_feat = self.qmodule(feat)
		feat = self.gunet.fuse_q(feat, q_feat=q_feat, feat_attn=self.feat_attn, q_feat_attn=self.q_feat_attn)
		x = self.gunet.decode(x, feat, output_skips)
		return x
...
if __name__ == '__main__':
    ...
    t = torch.randn(4, 172, 256, 256).cuda()
    model=QGUnet().cuda()
    print(model(t).shape)
